...
__global__ void calculate_s1(fingerprint* db, float* result) {
...
__global__ void calculate_s2(fingerprint* db, float* result, int* mapping) {
...
__global__ void calculate_s3(fingerprint* db, float* result, int* mapping) {
    int j = mapping[blockIdx.x];
...
__global__ void calculate_s4(fingerprint* db, float* result, int* mapping) {
    int j = mapping[blockIdx.x];
...
int main(int argc, char** argv) {
    ...
    std::vector<struct fingerprint> h_fp;
    int count_fp = read_from_file(h_fp, fp_filename);

    std::vector<float> local_orie, local_cohe, local_freq;
    get_fingerprint_local_values(h_fp[0], local_orie, local_cohe, local_freq);
    float avg_o = get_fingerprint_average_orientation(h_fp[0]);
    float avg_f = get_fingerprint_average_frequency(h_fp[0]);
    ...
    cudaMemcpyToSymbol(fp, &h_fp[0], sizeof(fingerprint));
    // auto timer_start = std::chrono::steady_clock::now();
    // std::cout << "Starting S1\n";
    calculate_s1<<<count_db,BLOCKSIZE>>>(d_db, d_s1_result);
    ...
    calculate_s2<<<count_db_fingerprint,BLOCKSIZE>>>(d_db, d_s2_result, d_mapping);
    ...
    calculate_s3<<<count_db_fingerprint,1>>>(d_db, d_s3_result,d_mapping);
    ...
    calculate_s4<<<count_db_fingerprint,1>>>(d_db, d_s4_result, d_mapping);
