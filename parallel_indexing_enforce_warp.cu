...
__global__ void calculate_s1(fingerprint* db, fingerprint* fp, float* result) {
    ...
    atomicAdd(&ssin, tsin);
    if (i < PADSIZE) {
        s = dbyte_to_coherence(fp->local_coherence[i+BLOCKSIZE])*dbyte_to_coherence((db+j)->local_coherence[i+BLOCKSIZE]);
        d = M_PI/180.0f * 2 * (dbyte_to_orientation(fp->local_orientation[i+BLOCKSIZE])-dbyte_to_orientation((db+j)->local_orientation[i+BLOCKSIZE]));
        tcos = s*cos(d);
        tsin = s*sin(d);
...
__global__ void calculate_s2(fingerprint* db, fingerprint* fp, float* result, int* mapping) {
    ...
    if (i < PADSIZE) {
        t_addition = dbyte_to_frequency(fp->local_frequency[i+BLOCKSIZE]) + dbyte_to_frequency((db+j)->local_frequency[i+BLOCKSIZE]);
        t_absdiff = abs(dbyte_to_frequency(fp->local_frequency[i+BLOCKSIZE]) - dbyte_to_frequency((db+j)->local_frequency[i+BLOCKSIZE]));
        atomicAdd(&s_addition, t_addition);
        atomicAdd(&s_absdiff, t_absdiff);
    }
    __syncthreads();
...
// nvcc -o parallel_indexing_enforce_warp parallel_indexing_enforce_warp.cu fingerprint_structure.cpp -std=c++11 -lineinfo
