...
__global__ void calculate_s2(float* s_addition, float* s_absdiff, float* result, int count) {
    int j = blockIdx.x*blockDim.x + threadIdx.x;
    if (j<count) result[j] = 1 - (s_absdiff[j]/s_addition[j]);
}
...
__global__ void calculate_s4(fingerprint* fp, unsigned char* global_orientations, float* result, int count) {
    int j = blockIdx.x*blockDim.x + threadIdx.x;
    if (j >= count) return;
    result[blockIdx.x] = 1-(abs(dbyte_to_orientation(fp->avg_orientation)-dbyte_to_orientation(global_orientations[j]))/180.0f);
}
...
int main(int argc, char** argv) {
    ...
    cudaMalloc((void **)&d_local_orientations, count_db*36*sizeof(float));
    cudaMalloc((void **)&d_local_coherences, count_db*36*sizeof(float));
    cudaMalloc((void **)&d_local_frequencies, count_db*36*sizeof(float));
    cudaMalloc((void **)&d_global_orientations, count_db*sizeof(float));
    cudaMalloc((void **)&d_global_frequencies, count_db*sizeof(float));
    ...
    cudaMemcpy(d_local_orientations, local_orientations, count_db*36*sizeof(float), cudaMemcpyHostToDevice);
    cudaMemcpy(d_local_coherences, local_coherences, count_db*36*sizeof(float), cudaMemcpyHostToDevice);
    cudaMemcpy(d_local_frequencies, local_frequencies, count_db*36*sizeof(float), cudaMemcpyHostToDevice);
    cudaMemcpy(d_global_orientations, global_orientations, count_db*sizeof(float), cudaMemcpyHostToDevice);
    cudaMemcpy(d_global_frequencies, global_frequencies, count_db*sizeof(float), cudaMemcpyHostToDevice);
    ...
    // S1
    calculate_s1_preparation<<<(count_db*36/256)+1, 256>>>(d_fp, d_local_orientations, d_local_coherences, d_s_sum, d_cos_sum, d_sin_sum, count_db);
    std::cerr << "s1 prep done\n";
    calculate_s1<<<(count_db/256)+1, 256>>>(d_s_sum, d_cos_sum, d_sin_sum, d_s1_result, count_db);
    std::cerr << "s1 done\n";
    gpuErrchk( cudaPeekAtLastError() );
    gpuErrchk( cudaDeviceSynchronize() );
    get_best_core_s1<<<(count_db/256)+1, 256>>>(d_core_ids, d_s1_result, d_mapping, count_db);
    ...
    calculate_s2_preparation<<<(count_db*36/256)+1, 256>>>(d_fp, d_local_frequencies, d_s_addition, d_s_absdiff, count_db);
    calculate_s2<<<(count_db/256)+1, 256>>>(d_s_addition, d_s_absdiff, d_s2_result, count_db);
    // cudaMemcpy(&s2_result[0], d_s2_result, count_db_fingerprint*sizeof(float), cudaMemcpyDeviceToHost);

    // S3
    calculate_s3<<<(count_db/256)+1, 256>>>(d_fp, d_global_frequencies, d_s3_result, count_db);
    // cudaMemcpy(&s3_result[0], d_s3_result, count_db*sizeof(float), cudaMemcpyDeviceToHost);

    // S4
    calculate_s4<<<(count_db/256)+1, 256>>>(d_fp, d_global_orientations, d_s4_result, count_db);
