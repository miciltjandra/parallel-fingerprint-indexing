...
int main(int argc, char** argv) {
    ...
    // S1
    calculate_s1<<<count_db,BLOCKSIZE, 12, streams[0]>>>(d_db, d_fp, d_s1_result);

    // S2
    calculate_s2<<<count_db,BLOCKSIZE, 8, streams[1]>>>(d_db, d_fp, d_s2_result);
    // cudaMemcpy(&s2_result[0], d_s2_result, count_db_fingerprint*sizeof(float), cudaMemcpyDeviceToHost);

    // S3
    calculate_s3<<<count_db,1, 0, streams[2]>>>(d_db, d_fp, d_s3_result);
    // cudaMemcpy(&s3_result[0], d_s3_result, count_db*sizeof(float), cudaMemcpyDeviceToHost);

    // S4
    calculate_s4<<<count_db,1, 0, streams[3]>>>(d_db, d_fp, d_s4_result);
...
// nvcc -o parallel_indexing_stream_all parallel_indexing_stream_all.cu fingerprint_structure.cpp -std=c++11 -lineinfo --default-stream per-thread
