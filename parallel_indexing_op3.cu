...
__global__ void calculate_s1(fingerprint* db, fingerprint* fp, float* result) {
    // Specialize BlockReduce for a 1D block of 36 threads on type float
    ...
    __syncthreads();
    float ss = BlockReduce(temp_storage).Sum(s);
    float ssin = BlockReduce(temp_storage).Sum(tsin);
    float scos = BlockReduce(temp_storage).Sum(tcos);
    if (i == 0) {
