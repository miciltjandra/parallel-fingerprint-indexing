...
struct d_fingerprint {
    int id;
    float local_orientation[36];
    float local_coherence[36];
    float local_frequency[36];
    float avg_orientation;
    float avg_frequency; 
};
...
__global__ void load_fp(fingerprint* fp, d_fingerprint* d_fp) {
    int i = threadIdx.x;
    d_fp->local_orientation[i] = dbyte_to_orientation(fp->local_orientation[i]);
    d_fp->local_coherence[i] = dbyte_to_coherence(fp->local_coherence[i]);
    d_fp->local_frequency[i] = dbyte_to_frequency(fp->local_frequency[i]);
    d_fp->avg_orientation = dbyte_to_orientation(fp->avg_orientation);
    d_fp->avg_frequency = dbyte_to_frequency(fp->avg_frequency);
}

__global__ void calculate_s1(fingerprint* db, d_fingerprint* fp, float* result) {
    ...
    __syncthreads();
    float s = fp->local_coherence[i]*dbyte_to_coherence((db+j)->local_coherence[i]);
    float d = M_PI/180.0f * 2 * (fp->local_orientation[i]-dbyte_to_orientation((db+j)->local_orientation[i]));
    float tcos = s*cos(d);
    float tsin = s*sin(d);
...
__global__ void calculate_s2(fingerprint* db, d_fingerprint* fp, float* result, int* mapping) {
    ...
    float t_addition = fp->local_frequency[i] + dbyte_to_frequency((db+j)->local_frequency[i]);
    float t_absdiff = abs(fp->local_frequency[i] - dbyte_to_frequency((db+j)->local_frequency[i]));
...
__global__ void calculate_s3(fingerprint* db, d_fingerprint* fp, float* result, int* mapping) {
    int j = mapping[blockIdx.x];
    result[blockIdx.x] = 1 - (abs(fp->avg_frequency-dbyte_to_frequency((db+j)->avg_frequency))/max(fp->avg_frequency, dbyte_to_frequency((db+j)->avg_frequency)));
}

__global__ void calculate_s4(fingerprint* db, d_fingerprint* fp, float* result, int* mapping) {
    int j = mapping[blockIdx.x];
    result[blockIdx.x] = 1-(abs(fp->avg_orientation-dbyte_to_orientation((db+j)->avg_orientation))/180.0f);
}
...
int main(int argc, char** argv) {
    ...
    // Preparing memory
    fingerprint *d_fp_unloaded, *d_db;
    d_fingerprint *d_fp;
    std::vector<float> result(count_db_fingerprint, 0);
    float *d_s1_result, *d_s2_result, *d_s3_result, *d_s4_result, *d_result;
    
    cudaMalloc((void **)&d_fp_unloaded, sizeof(fingerprint));
    cudaMalloc((void **)&d_fp, sizeof(d_fingerprint));
    ...
    cudaMemcpy(d_fp_unloaded, &fp[0], sizeof(fingerprint), cudaMemcpyHostToDevice);

    // Load FP
    load_fp<<<1, BLOCKSIZE>>>(d_fp_unloaded, d_fp);
...
// nvcc -o parallel_indexing_load_fp parallel_indexing_load_fp.cu fingerprint_structure.cpp -std=c++11 -lineinfo
