...
__global__ void calculate_s1_preparation(fingerprint* db, fingerprint* fp, float* g_s, float* g_cos, float* g_sin) {
    ...
    g_s[j*blockDim.x+i] = s;
    g_cos[j*blockDim.x+i] = tcos;
    g_sin[j*blockDim.x+i] = tsin;
}
...
// nvcc -o parallel_indexing_split_s1_v2 parallel_indexing_split_s1_v2.cu fingerprint_structure.cpp -std=c++11 -lineinfo
