...
__global__ void calculate_s1_preparation(fingerprint* fp, unsigned char* orientations, unsigned char* coherences, float* g_s, float* g_cos, float* g_sin, int count) {
...
__global__ void calculate_s1(float* s, float* cos, float* sin, float* result, int count) {
    int j = blockIdx.x*blockDim.x + threadIdx.x;
    if (j >= count) return;
    int local_start = j*36;
    float ss = 0.0f, scos = 0.0f, ssin = 0.0f;
    for (int i=0 ; i<36 ; i++) {
        ss += s[local_start+i];
        scos += cos[local_start+i];
        ssin += sin[local_start+i];
    }
...
int main(int argc, char** argv) {
    ...
    calculate_s1_preparation<<<((count_db*36)/256)+1, 256, 32, streams[0]>>>(d_fp, d_local_orientations, d_local_coherences, d_s, d_cos, d_sin, count_db);
    std::cerr << "s1 prep done\n";
    calculate_s1<<<(count_db/256)+1, 256, 4, streams[0]>>>(d_s, d_cos, d_sin, d_s1_result, count_db);
    std::cerr << "s1 done\n";
...
// nvcc -o parallel_indexing_diffstructure_v2 parallel_indexing_diffstructure_v2.cu fingerprint_structure.cpp -std=c++11 -lineinfo
